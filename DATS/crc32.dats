...
extern
castfn witness(uint8) : [ i : nat | i >= 0 && i < 256 ] uint(i)
...
fn crc32 {l:addr}{m:nat}(pf : !bytes_v(l, m) | p : ptr(l), l : size_t(m)) : uint32 =
  let
    var crc32_ret: uint32 = uint2uint32(0xFFFFFFFFu)
    var crc32_table = @[uint][256]( 0x00000000u
                                  , 0x77073096u
                                  , 0xEE0E612Cu
                                  , 0x990951BAu
                                  , 0x076DC419u
    ...
    val () = for* { i : nat | i <= m } .<m-i>. (i : size_t(i)) =>
        ...
        let
          ...
          var crc_trunc = uint2uint8(crc32_ret)
          var ix = witness(g0uint_lxor_uint8(crc_trunc, current_byte))
          var crc_shift = g0uint_lsr(crc32_ret, 8)
          val lookup: uint = $UN.cast(array_get_at_guint(crc32_table, ix))
          val () = crc32_ret := g0uint_lxor_uint32(uint2uint32(lookup), crc_shift)
        in end
  in
    g0uint_lxor_uint32(crc32_ret, uint2uint32(0xFFFFFFFFu))
  end
